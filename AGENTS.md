...
`terra-mcp`: MCP server exposing **Google Drive + Sheets** over stdio. Auth = **OAuth user login**
...
- **Server-side** (`TERRA_MCP_SAFE_MODE=1`): `selectGoogleTools(true)` registers only read-only tools.
...
`terra-mcp client` snippet and/or `TERRA_MCP_SAFE_MODE=1`.
